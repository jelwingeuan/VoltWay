...
VoltWay is an iOS 26+ and CarPlay pilot for finding compatible Gentari EV chargers in Malaysia, viewing normalized availability and pricing, saving favorites, and handing navigation to Apple Maps. Its iPhone controls use native Liquid Glass while charger data remains on solid, readable surfaces.
...
The simulator suite covers discovery search and availability filtering, estimate eligibility and rounding, route-corridor matching/order, nearby-alternative ranking and radius, model freshness, token refresh, list-fetch time on success/failure/profile changes, request privacy, profile rollback, and snapshot clearing with mocked responses. On an iOS 26+ iPhone Simulator, also inspect Explore map and list, charger details and alternatives, refresh-error and empty states, demo labeling, Trip, Saved, account, vehicle setup, and authentication in light/dark appearance, large text, Reduce Motion, Reduce Transparency, and Increase Contrast. Check glass controls over varied map imagery, keyboard and sheet behavior, stale status and price labels, denied location and no-destination/no-route/no-stops states, and Apple Maps handoff. Live Supabase authentication and row-level isolation require a configured project; live availability/pricing require the written partner agreement and feed. CarPlay scene behavior requires Apple’s approved EV-charging entitlement and a matching provisioning profile. None of those external integrations are validated by the mocked suite. Charging activation, payments, reservations, vehicle telemetry, custom navigation, and multi-network aggregation are outside this pilot.
