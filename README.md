...
The simulator suite covers discovery search and availability filtering, estimate eligibility and rounding, route-corridor matching/order, model freshness, token refresh, request privacy, profile rollback, and snapshot clearing with mocked responses. On an iPhone Simulator, also inspect List/Map and trip route states in light/dark appearance and large text, verify denied location and no-destination/no-route/no-stops states, and confirm Apple Maps handoff. Live Supabase authentication and row-level isolation require a configured project; live availability/pricing require the written partner agreement and feed. CarPlay scene behavior requires Apple’s approved EV-charging entitlement and a matching provisioning profile. None of those external integrations are validated by the mocked suite. Charging activation, payments, reservations, vehicle telemetry, custom navigation, and multi-network aggregation are outside this pilot.
